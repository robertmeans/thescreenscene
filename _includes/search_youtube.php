<div id="youtube" class="searches bottom">
<form name="youtube" action="https://www.youtube.com/results" method="get" target="_blank">

  <p>YouTube</p>
    <div class="srs"><input id="sr_05" name="search_query" type="text"> <a onclick="reset_yt();"><i class="fas fa-backspace"></i></a><a data-role="srcb" data-id="sr_05" class="srcb static"><i class="far fa-copy fa-fw"></i></a></div>

    <div class="check-group">
    	<div class="chk-yt"></div>
    </div>

    <div class="go"><a href="#" class="go-a static" onclick="$(this).closest('form').submit()">Go</a></div>
</form> 
</div><!-- end YouTube -->
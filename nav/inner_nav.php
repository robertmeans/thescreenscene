<?php
if ($inner_nav_context == "owner") { $oos = "1"; } else { $oos = "0"; } /* $oos = owner or shared */

if ($inner_nav_context == "cp-not-found") { ?>
  <li>
...
<?php } if ($layout_context == 'homepage' && ($oos == "1" || $row['edit'] == "1")) { ?>
  <li>
    <form id="et-form" action="" method="post">
      <input type="hidden" id="ownShare" name="ownShare" value="<?= $oos; ?>"><?php /* $oos: see top of this pg */ ?>
...
<?php if ($layout_context != 'edit_searches' && $layout_context != 'new_project') { ?>
  <li>
    <form class="gth" method="post">
      <input type="hidden" name="organizesearchfields" value="1"><?php /* key */ ?>
      <a class="osf-link"><div class="tooltip"><span class="tooltiptext">Settings</span><i class="fas fa-cog fa-fw"></i></div></a>
    </form>
  </li>
<?php } ?> 

<?php if ($oos == "1" && ($layout_context != 'edit_order' && $layout_context != 'new_project')) { ?>
  <li>
    <form class="gth" method="post">
...
<?php if (($oos == "1" || $row['share'] == "1") && $layout_context != 'share_project' && $layout_context != 'new_project') { ?>
  <li>
    <form method="post">
  ...
  <li>
    ...
    <ul>
      ...
      <form action="" method="post">
        ...
        <input type="hidden" name="owner" value="<?= $oos; ?>">
...
<?php if ($oos == "1" && $layout_context == 'delete_project') { ?>
...
} /* end if/else: ($inner_nav_context == "cp-not-found") */  ?>
